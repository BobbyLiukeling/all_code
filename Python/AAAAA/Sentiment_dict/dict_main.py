...
def single_review_sentiment_score(weibo_sent):
	...
	cuted_review = tp.cut_sentence(weibo_sent)  # 句子切分(用逗号等符号），单独对每个句子进行分析
	'''
	'这手机的画面很好，操作很流畅。'  切分为：
	['这手机的画面很好，', '操作很流畅。']
	'''
...
def run_score():
	fp_test = open('a.txt', 'r',encoding='utf-8')   # 待处理数据
	contents = []
	for content in fp_test.readlines():  #读取文件所有数据
		content = content.strip() #去除字符串的空格
