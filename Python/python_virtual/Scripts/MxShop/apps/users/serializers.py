...
class UserDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ("username", "gender", "birthday", "email", "mobile",'owner')
...
class UserRegSerializer(serializers.ModelSerializer):
    ...
    #将密码设置为密文 write_only将返回到前端设置为false，这样前端就会将密码隐藏
    ...
    def validate_code(self, code):
        ...
        if verify_records:
            ...
            # 有效期为五分钟。
            mintes_ago = datetime.now() - timedelta(hours=0, minutes=30, seconds=0)
            if mintes_ago > last_record.add_time:
    ...
    # 不加字段名的验证器作用于所有字段之上。attrs是字段 validate之后返回的总的dict,验证完code后将数据库中的code删除掉
