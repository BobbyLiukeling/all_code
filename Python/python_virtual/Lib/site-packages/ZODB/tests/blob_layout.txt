...
Traceback (most recent call last):
ValueError: Directory layout `lawn` selected for blob directory ./static/blobs/, but marker found for layout `bushy`
>>> rmtree(blobs)
...
... # doctest: +ELLIPSIS
Traceback (most recent call last):
ValueError: Directory layout `bushy` selected for blob directory ./static/blobs/, but marker found for layout `lawn`
...
Migrating blob data from `./static/old` (lawn) to `./static/bushy` (bushy)
    OID: 0x0a - 2 files
    OID: 0x1b7a - 2 files
    OID: 0x1b7f - 2 files
...
Migrating blob data from `./static/bushy` (bushy) to `./static/lawn` (lawn)
   OID: 0x0a - 2 files
   OID: 0x1b7a - 2 files
   OID: 0x1b7f - 2 files
