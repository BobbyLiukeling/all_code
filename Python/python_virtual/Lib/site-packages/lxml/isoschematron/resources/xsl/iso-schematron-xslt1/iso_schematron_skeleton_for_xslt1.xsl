  ...
  VERSION INFORMATION 
   ...
   Version:2006-11-07 (ISO: first release private to schematron-love-in maillist for review)
    ...
    KNOWN TYPICAL LIMITATIONS:
      * Don't use <sch:ns prefix="xsl" ./static/> with a namespace other than the standard
...
<!-- e.g. saxon file.xml file.xsl "sch.exslt.imports=./static/string.xsl;./static/math.xsl" -->
...
<!--
			<axsl:choose>
				<axsl:when test="count(. | /static/namespace::*) = count(/static/namespace::*)">
...
<!-- ============================================================== -->
	...
	<xsl:template match="iso:active">
                ...
                <xsl:if test="not(/static//static/iso:pattern[@id = current()/@pattern])
                and not(/static//static/iso:include)">
	...
	<xsl:template  match="xsl:key" mode="do-keys" >
	     ...
	     <xsl:choose>
	       ...
	       <xsl:choose>
	       	<xsl:when test="@path">
				<axsl:key match="{/static/@context}" name="{@name}" use="{@path}"/>
			</xsl:when>
			<xsl:otherwise>
							<axsl:key match="{/static/@context}" name="{@name}" use="{@use}"/>
			</xsl:otherwise>
			</xsl:choose>	
		</xsl:when>
		<xsl:otherwise>
   ...
   <xsl:template match="iso:include" mode="do-all-patterns">
	...
	or (/static/iso:phase[@id= $phase]/iso:active[@pattern= current()/@id])">
	...
	<xsl:template match="iso:rule[not(@abstract='true')] ">
		...
		<xsl:if test="$select-contexts='key'">
		    <axsl:key name="M"
			      match="{@context}" 
			      use="'M{count(/static/preceding-sibling::*)}'"/>
		</xsl:if>
...
<!-- DPC priorities count up from 1000 not down from 4000 (templates in same priority order as before) -->
		...
		priority="{1000 + count(following-sibling::*)}" mode="M{count(/static/preceding-sibling::*)}">
			...
			<xsl:if test="not($select-contexts)">
			  <axsl:apply-templates select="{$context-xpath}" mode="M{count(/static/preceding-sibling::*)}"/>
			</xsl:if>
		</axsl:template>
	</xsl:template>
